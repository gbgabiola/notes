...
- [Django 2.x Cheat Sheet](#django-2.x-cheat-sheet)
...
- [Docker Commands, Help & Tips](#docker-commands,-help-&-tips)
...
```sh
> cat ~/.ssh/id_rsa.pub | ssh genesis@192.168.1.29 "mkdir -p ~/.ssh && chmod 700 ~/.ssh && cat >>  ~/.ssh/authorized_keys
```

#### Create & copy a file to the server using SCP
```sh
$ touch test.txt
$ scp ~/test.txt genesis@192.168.1.29:~
```
...
#### Login as genesis
```sh
> ssh genesis@doserver
```

#### We need to add the key to genesis .ssh on the server, log back in as root
```sh
$ ssh root@doserver
$ cd /home/genesis
$ mkdir .ssh
...
```
(paste in the id_rsa_do.pub key, exit and log in as genesis)
...
#### Change owner of /home/genesis/* to genesis
```sh
$ sudo chown -R genesis:genesis /home/genesis
```

#### May need to set permission
```sh
$ chmod 700 /home/genesis/.ssh
```
...
#### Clone repo
```sh
$ git clone git@github.com:genesisgabiola/react_otka_auth.git
```
...
```sh
$ sudo mv -v /home/genesisgabiola/react_otka_auth/build/* /var/www/html
```
...
- ... run -v /Users/genesis/stuff:/path/container (mac/linux)
- ... run -v //c/Users/genesis/stuff:/path/container (windows)
